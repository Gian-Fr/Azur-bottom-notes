 ...
 */
class pdf_azur_bottom_notes extends ModelePDFPropales
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "azur bottom notes";
		$this->description = 'Azur Model with public notes on bottom instead of top';
		...
		if ($conf->propal->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
                ...
                if ($notetoshow) {
                    ...
                    $pdf->SetFont('', '', $default_font_size - 1);
                    $pdf->writeHTMLCell(190, 3, $this->posxdesc - 1, $bottomlasttab, dol_htmlentitiesbr($notetoshow), 0, 1);
                    $nexY = $pdf->GetY();
                    $height_note = $nexY - $tab_top;

                    // Rect takes a length in 3rd parameter
                    $pdf->SetDrawColor(192, 192, 192);
                    $pdf->Rect($this->marge_gauche, $bottomlasttab-1, $this->page_largeur - $this->marge_gauche - $this->marge_droite, 6);
